...
async function sendToPrintAPI(postcardData) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to send to print API. Status: ${response.status}. Body: ${errorBody}`);
  }
  ...
  try {
    ...
    const { error: insertError } = await supabase
      ...
      .insert({
        ...
        front_image_url: postcardData.frontImageUrl,
        back_image_url: postcardData.backImageUrl
      });

    if (insertError) {
      console.error('Error logging postcard:', insertError);
    }
      ...
      html: `
        <div style="font-family: sans-serif; text-align: center; padding: 20px;">
          ...
          <a href="${confirmationEmailConfig.promoLinkURL}" target="_blank">
            <img src="${confirmationEmailConfig.promoImageURL}" alt="Promo" style="max-width: 100%; width: 300px; margin-top: 10px; border-radius: 8px;">
          </a>
        </div>
      `
    };
    ...
    if (error instanceof jwt.JsonWebTokenError) {
      return response.status(401).send('<h1>Error</h1><p>Your verification link is invalid or has expired.</p>');
    }
    return response.status(500).send(`<h1>Error</h1><p>An unexpected error occurred. ${error.message}</p>`);
  }
}
