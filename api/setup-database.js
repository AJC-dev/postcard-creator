...
export default async function handler(req, res) {
  ...
  // Basic auth/guard: in production you should protect this endpoint.
  // For now we allow it but you should remove or protect after running once.
  try {
    const pool = getPool();
    const sql = pool.sql;

    // Run statements in sequence. These are idempotent.
    await sql`
    ...
    `;
    // Insert default config only if not present
    const settings = JSON.stringify(fallbackConfig);
    await sql`
...
Because this script now requires a `POST` request, you can no longer simply visit the URL in your browser to run it. After you deploy this new file, you will need to use a command in your Chromebook's Terminal to run it **one time**.

1.  **Open the Terminal** on your Chromebook.
2.  Copy and paste the following `curl` command, making sure to replace `your-project-url.vercel.app` with your actual Vercel deployment URL:

    ```bash
    curl -X POST https://your-project-url.vercel.app/api/setup-database
